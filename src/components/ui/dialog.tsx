      ...
      >
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            className="text-text-muted hover:text-text hover:bg-surface-elevated focus:ring-primary focus:ring-offset-surface absolute top-4 right-4 rounded-full p-1.5 transition-colors focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:pointer-events-none"
          >
            <XIcon size={16} />
...
function DialogTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title
      ...
      className={cn('text-text text-lg leading-tight font-semibold', className)}
      {...props}
    />
  );
}
